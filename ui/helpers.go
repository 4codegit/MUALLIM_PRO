...
// ParseShortNameToGrade extracts a numeric grade (2-10) from a mark's
// shortName string. Handles:
//   - "8"     → 8
//   - "8/10"  → 8 (fractional absent form — take numerator)
//   - "0/X"   → -1 (absent — caller should skip)
//   - ""      → -1 (no mark)
//   - "ғ/у"   → -1 (non-numeric — skip)
//
// Returns -1 if the value can't be parsed as a 2-10 grade.
func ParseShortNameToGrade(shortName string) int {
        s := shortName
        if s == "" {
                return -1
        }
        // Handle fractional form (absent): "0/X" → skip; "N/X" → N
